...
- **AI 对话风格规范**：与维护者中文对话时可采用口语化、幽默风格（参考：沈腾式）；但所有正式产出（设计文档、部署文档、CHANGELOG、commit message、代码注释、docstring、API 文档）必须使用标准技术用语，禁止口语化、梗化、非正式表达。
...
- **第三方成熟实现优先直接复用**：
  - 若 Vibe-Trading 或其他成熟项目已有对应功能实现（如 Persistent Memory、Research Goal、Shadow Account 等），**必须直接复用其代码**，而非自行重新实现
  - 复用原则：保持接口兼容的前提下，最小化适配修改（如修改路径前缀、配置项名称等）
  - 避免重复造轮子导致的过度调试和维护负担
  - 复用时需在代码注释中标注来源（如 `Adapted from Vibe-Trading`）并保留原文件版权信息
