...
- [修复] 情报中心近期新闻在暂无已分类高优先级结果时回退展示近期缓存新闻，并对淘金列表活跃候选按股票与市场去重。
...
- [改进] 模拟交易信号显式记录技术面、Sentinel 情报和风险权重，并将止盈从硬性全仓卖出调整为基于趋势与 Sentinel 情绪的继续持有/卖出复核。
- [改进] 模拟交易当前持仓列表在盈亏比例外新增盈亏金额列，便于直接查看浮动盈亏数量。
- [改进] 模拟交易当前持仓列表拆分为 A 股与美股两个表格，并在各市场表内独立排序，避免不同市场持仓混排。
...
- [新功能] 模拟交易新增"空仓过夜"模式：启用后在收盘前 N 分钟（可配置，默认 15 分钟）自动以市价清仓所有持仓，避免持仓隔夜风险。
- [新功能] 模拟交易 AI 扫描间隔支持按账户自定义（1-60 分钟），覆盖全局环境变量默认值，在策略设置页直接配置。
...
- [新功能] Web 新增“淘金列表”页面，展示当前活跃候选、加入理由、来源、有效期和自动交易状态，并提供加入/过期/移出变更历史与原因追踪。
- [改进] 盘后淘金触发将 Scanner、沙里淘金和热点雷达拆为独立任务源，CN/US 两个收盘窗口分别启动对应市场任务，单个来源失败不会阻断其他来源写入淘金列表。
- [改进] 淘金列表自动交易新增动态止损/止盈开关，可启用 AI/技术分析给出的绝对退出价，并在价格方向不合理时回退到固定百分比。
- [改进] 自选股和淘金列表页面新增实时价格刷新，列表接口支持 refresh=true 返回当前价、涨跌幅、行情来源和数据更新时间，前端在页面可见、窗口聚焦和低频兜底轮询时更新。
- [改进] 自选股和淘金列表行情刷新新增最近报价缓存，闭市市场不再请求实时行情源，但会展示最后一次成功获取的价格、来源和时间。
...
- [修复] 修复自选股最近分析状态未从分析历史读取的问题，删除 Analyze List 历史后列表会同步回到未分析状态。
- [修复] 修复沙里淘金 A 股候选可能因主题区域缺失或最终 TopN 排序被美股全部挤出的问题，A 股候选新增主题兜底并在最终推荐中按市场保留名额。
- [修复] 修复 A 股分析遇到 LLM 非 JSON 响应时直接请求失败的问题，现在会保存带风险提示的降级兜底分析结果。
- [改进] LLM 调用日志新增每个模型请求的开始与结束记录，包含耗时、输出长度、调用模式和失败原因，便于排查本地模型卡顿。
- [改进] 股票分析 LLM 输出新增 JSON-only 硬约束，并在模型返回自然语言时先尝试一次 JSON 修复调用，修复失败后才保存降级兜底结果。
- [修复] 修复模拟交易信号生成在 LLM 返回空响应时把 `None` 当字符串解析导致二次异常的问题，现在会降级为 skip 信号。
- [修复] 修复模拟交易在仅 A 股开市时仍扫描美股自选股的问题，交易时段限制开启后每轮只处理当前开市市场的股票。
- [修复] 修复模拟交易自动任务后台执行时 Web 立即运行按钮仍显示可执行的问题，状态接口新增当前周期执行中标记并同步到前端按钮状态。
- [改进] 模拟交易自动交易页面新增独立状态徽标，区分“自动监控中”和“立即运行”手动动作，避免按钮文案被误认为自动交易状态。
- [改进] 模拟交易持仓价格刷新改为优先使用实时行情，并在 Web 页面每 5 秒刷新开市持仓实时价、每 60 秒刷新账户权益与自动交易状态。
- [改进] 模拟交易账户概览新增按开仓/平仓聚合的通用交易历史，统一展示手动与自动交易的数量、买入价、卖出价、盈亏、账户、来源和 AI/风控原因，并在持仓表展示每只在手股票当前总价；自动交易下单改为使用下单时最新撮合价，避免信号历史价与实时风控价不一致导致反复高买低卖。
- [改进] 模拟交易交易历史支持按每个展示字段排序，默认按交易时间倒序展示。
- [改进] 模拟交易账户概览按 CNY / USD 分开展示账户总值与独立可用现金卡片，并基于资金流水净投入展示分币种总盈亏和收益率。
- [改进] 模拟交易账户概览净值走势改为真实折线图，新增人民币总值、美元总值、人民币收益率、美元收益率四条趋势线，并支持鼠标悬停查看单日明细；旧快照缺少分币种字段时会按成交流水和历史收盘价回算展示。
- [改进] 模拟交易自动交易增加交易前止损/止盈风控、止损/亏损卖出后同股买入冷却期，以及 AI 信号风险标记展示，降低短时间反复高买低卖风险。
- [修复] 模拟交易自动挂单成交前会重新校验冷却期并取消超时旧挂单；自动买入限价高于当前价时取消并等待下一轮重新评估，避免旧 AI 信号在稍后用过期高价买入。
- [修复] 模拟交易最大回撤保护改为基于资金流调整后的收益率净值曲线计算，避免入金、出金或数据修正导致原始资产峰值回撤误触发暂停。
- [改进] 模拟交易持仓刷新按市场开闭市分频，开市市场正常刷新，闭市市场低频刷新，并在自动风控中跳过闭市市场，减少非当前交易市场的数据源噪音。
- [新功能] 自选股新增美股代码搜索建议，输入公司名或常见别名（如 GOOGLE）时可选择正确股票代码后添加。
- [修复] 自选股美股代码建议框改为页面顶层浮层，避免被卡片容器裁剪或遮挡。
- [修复] 模拟交易闭市市场持仓刷新会记录刷新尝试时间，避免价格未变化时反复请求同一闭市市场行情。
- [改进] Docker 默认时区改为 `America/Los_Angeles`，使容器日志与太平洋时间对齐。
- [修复] 模拟交易自动卖出增加 AI 二次复核：首轮 AI 给出卖出后，执行前会再次让 AI 综合趋势、新闻、持仓、仓位风险决定全卖/减仓/不卖，复核失败或不批准时不执行卖出。
- [修复] 修复模拟交易自动信号读取 News Sentinel 时调用缺失接口导致新闻情绪始终显示“情报中心不可用”的问题，现在会按股票代码、名称和受影响个股匹配近期已分类新闻。
- [改进] News Sentinel 定向抓取会保存目标股票代码和名称，分类 Prompt 与单股检索统一使用该上下文，减少自选股新闻漏取或串股；同时修复 HTML Spider 健康状态钩子缺失导致监管/宏观抓取异常的问题。
- [修复] 模拟交易自动信号生成遇到本地日线技术指标缺失或超过 3 天时会先同步刷新并保存最新日线，刷新后仍无有效数据才跳过，避免用过期 MA/量比/5日涨跌做 AI 技术分析。
- [改进] 自选股新增或更新时会后台预热该股票近期日线缓存，减少 watchlist 变更后行情缓存长期停留在旧价格的风险；删除自选股后自动交易下一轮会重新读取列表并停止扫描该股票。
...
- [改进] News Sentinel 缓存新鲜度保护：`SentinelCacheClient.is_available()` 新增基于 `spider_runs` 表的最后成功抓取时间检查（默认 4 小时阈值，可通过 `SENTINEL_MAX_CACHE_AGE_HOURS` 调整）；Sentinel Docker 停止运行后超过阈值，主服务自动回退到在线搜索，避免使用过期缓存。
- [新功能] News Sentinel 即时新闻预取：新增 `POST /api/v1/sentinel/fetch-now` 端点，对单只股票立即执行定向抓取 + LLM 分类并将股票注册到关注列表，用于用户首次分析某股票时实时预热缓存；`PUT /watched-stocks?merge=true` 支持追加模式，Scanner 可在不清空手动自选股的情况下批量注入候选股池；SearchService 缓存未命中时自动将股票注册到关注列表，确保下次分析时享有定向缓存。
...
- [新功能] 模拟交易自动交易支持交易时段感知：默认仅在 A 股（09:30-11:30 / 13:00-15:00 CST）或美股（09:30-16:00 ET）开市时段执行循环，非交易时段自动跳过；可通过 `SIMTRADE_MARKET_HOURS_ONLY=false` 禁用；前端自动交易面板实时显示 A 股/美股开闭市状态。
- [改进] 模拟交易"立即运行"改为异步触发：`POST /auto-trade/run` 立即返回 job_id（202），前端每 2 秒轮询 `GET /auto-trade/run/{job_id}` 直至完成，按钮不再长期卡在"执行中"状态；最长 4 分钟超时后自动解锁。
- [修复] 模拟交易默认账户读取兼容历史重复账户数据，并降低首次并发请求创建重复账户后导致 `/api/v1/sim-trade/*` 返回 500 的风险。
- [改进] 模拟交易入金/出金金额改为整数输入和整数校验，避免资金管理出现分单位金额。
- [修复] 修复模拟交易自动交易开关成功启动后仍返回失败的问题，并将默认自动交易调度间隔调整为 5 分钟。
